package kaajjo.thousandscourses.data.network.repository

import android.util.Log
import com.google.gson.Gson
import kaajjo.thousandscourses.data.network.api.CoursesApiService
import kaajjo.thousandscourses.data.network.dto.CoursesResponse
import kaajjo.thousandscourses.data.network.mapper.toDomain
...
class CoursesRepositoryImpl @Inject constructor(
    ...
    override suspend fun getCourses(): Flow<List<Course>> = flow {
        ...
        val gson = Gson()
        val courses = gson.fromJson(JSON_MOCK, CoursesResponse::class.java).courses.map { it.toDomain() }
        emit(courses)
    }.flowOn(Dispatchers.IO)
}


private const val JSON_MOCK = """
    {
  "courses": [
    {
      "id": 100,
